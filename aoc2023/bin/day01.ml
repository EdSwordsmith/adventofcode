...
let parse_line' line =
  ...
  let parse_digit s =
    ...
    | [] -> (None, [])
    | h :: tl -> (
        match int_of_char_opt h with
        | Some v -> (Some v, tl)
        | None -> (parse_written s, tl))
  in

  let rec parse_digits s acc =
    match s with
    | [] -> acc
    | _ -> (
        let d, s' = parse_digit s in
        match (acc, d) with
        | _, None -> parse_digits s' acc
        | (None, _), Some _ -> parse_digits s' (d, d)
        | (Some v, _), Some _ -> parse_digits s' (Some v, d))
  in
...
let solve () =
  ...
  let rec solve_helper (acc, acc') =
    match maybe_read_line () with
    | None -> (acc, acc')
    | Some line ->
        let v, v' = (parse_line line, parse_line' line) in
        solve_helper (v + acc, v' + acc')
  in
  solve_helper (0, 0)
